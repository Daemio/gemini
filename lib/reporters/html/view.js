...
module.exports = {
     ...
     */
    save: function(html) {
        const clientScript = browserify(path.resolve(__dirname, 'static', 'report.js'));
        return fs.mkdirsAsync(REPORT_DIR)
            .then(() => Promise.fromCallback(clientScript.bundle.bind(clientScript)))
            .then((buf) => {
                return Promise.all([
                    fs.writeFileAsync(makeOutFilePath('index.html'), html, 'utf8'),
                    fs.writeFileAsync(makeOutFilePath('report.js'), buf, 'utf8'),
